class Simulation {
  ...
  make2Darray() {
    ...
    for (
      let size = 0;
      size < Math.floor(this.height / this.resolution);
      size++
    ) {
      let row = [];
  ...
  initialDraw() {
    for (let y = 0; y < this.height; y += this.resolution) {
      for (let x = 0; x < this.width; x += this.resolution) {
        ctx.fillStyle =
          this.particles[Math.floor(y / this.resolution)][
  ...
  simulateTraits(y, x, secondY, secondX) {
    ...
    if (
      this.particles[y][x].traits.traits[
        this.particles[secondY][secondX].effect
      ] > Math.floor(Math.random() * 10)
    ) {
      switch (this.particles[secondY][secondX].effect) {
        case "flammable":
          let newFire = new Particle(
            "FIRE",
            "#fa301e",
            2,
            "smoke",
  ...
  inRange(y, x) {
    return (
      y >= 0 &&
      x >= 0 &&
      y < this.height / this.resolution &&
      x < this.width / this.resolution
    );
  }
      ...
      for (let x = this.width - this.resolution; x >= 0; x -= this.resolution) {
        ...
        if (this.particles[newY][newX].lifeTimeUpdate != this.updated) {
          ...
          if (this.particles[newY][newX].lifeTime == 0) {
            this.particles[newY][newX].changeParticle(
              this.particles[newY][newX].replace
            );
          }
  ...
  drawElements() {
    ctx.fillStyle = "black";
    ctx.fillRect(0, this.height, this.width, 60);
    for (let ind in this.elements) {
      ctx.fillStyle = this.elements[ind].color;
      ctx.fillRect(
        ind * this.elementBlockSize,
        this.height,
        this.elementBlockSize,
        this.elementBlockSize
      );
      ...
      ctx.font = "10px Arial";
      ctx.fillText(
        this.elements[ind].type,
        ind * this.elementBlockSize,
        this.height + this.elementBlockSize + 10
      );
    }
  }
  ...
  drawSimulation() {
    ...
    for (let y = 0; y < simulation.height; y += simulation.resolution) {
      for (let x = 0; x < simulation.width; x += simulation.resolution) {
        if (
          simulation.particles[Math.floor(y / simulation.resolution)][
            Math.floor(x / simulation.resolution)
          ].color !=
          simulation.oldFrame[Math.floor(y / simulation.resolution)][
            Math.floor(x / simulation.resolution)
          ].color
        ) {
          simulation.oldFrame[Math.floor(y / simulation.resolution)][
  ...
  downMove(y, x) {
    if (
      ...
      x < Math.floor(this.width / this.resolution) - 1 &&
      this.particles[y + 1][x + 1].level < this.particles[y][x].level &&
      this.particles[y + 1][x + 1].updated != this.updated
    ) {
      this.swapParticles(y + 1, x + 1, y, x);
  ...
  waterDownMove(y, x, orientation) {
    if (
      this.particles[y + orientation][x].level < this.particles[y][x].level &&
      this.particles[y + orientation][x].updated != this.updated
    ) {
      this.swapParticles(y + orientation, x, y, x);
    } else if (
      x > 0 &&
      this.particles[y + orientation][x - 1].level <
        this.particles[y][x].level &&
  ...
  sideMove(y, x) {
    if (this.particles[y][x].leftDirection) {
      if (
        ...
        this.particles[y][x].leftDirection =
          !this.particles[y][x].leftDirection;
      }
    } else {
      if (
  ...
  brickLike(y, x) {
    if (
      y < Math.floor(this.height / this.resolution) - 1 &&
      this.particles[y + 1][x].level < this.particles[y][x].level &&
      this.particles[y + 1][x].updated != this.updated
    ) {
      this.swapParticles(y + 1, x, y, x);
