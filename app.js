...
function pickElement(evt){
    ...
    if(Math.floor(evt.clientX / simulation.resolution) >= Math.floor(simulation.width / simulation.resolution) 
    || Math.floor(evt.clientY / simulation.resolution) <= Math.floor(simulation.height / simulation.resolution)){
        return;
    }
...
function drawParticle(evt){
    ...
    if(simulation.drawing && (simulation.particles[Math.floor(evt.clientY / simulation.resolution)][Math.floor(evt.clientX / simulation.resolution)].type == "AIR" || simulation.element.type == "AIR")){
        simulation.particles[Math.floor(evt.clientY / simulation.resolution)][Math.floor(evt.clientX / simulation.resolution)].changeParticle(simulation.element)
     }
}
